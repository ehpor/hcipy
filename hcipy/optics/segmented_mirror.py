...
def get_atlast_aperture(normalized=False, with_segment_gaps=True, segment_transmissions=1, write_to_disk=False, outDir=None):
    ...
    if write_to_disk:
        pupil_grid = make_pupil_grid(dims=pupil_size, diameter=pupil_diameter)
        atlast = evaluate_supersampled(func, pupil_grid, 8)

        imshow_field(atlast)
        for i in range(36):
            plt.annotate(str(i + 1), size='x-large', xy=(segment_positions.x[i]-pupil_diameter*0.03,
        ...
        write_fits(atlast.shaped, os.path.join(outDir, 'pupil.fits'))
