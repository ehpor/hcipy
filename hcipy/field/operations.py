from .field_base import Field, is_field
import numpy as np
import string
...
def field_einsum(subscripts, *operands, **kwargs):
	...
	'''
	op_is_field = [is_field(op) for op in operands]
	if not any(op_is_field):
		return np.einsum(subscripts, *operands, **kwargs)

	field_sizes = [o.grid.size for i, o in enumerate(operands) if op_is_field[i]]
	element_sizes = [o.shape[-1] for i, o in enumerate(operands) if op_is_field[i]]
	...
	ss = [s + unused_index if op_is_field[i] else s for i,s in enumerate(ss)]
	...
	grid = operands[np.flatnonzero(np.array(op_is_field))[0]].grid
