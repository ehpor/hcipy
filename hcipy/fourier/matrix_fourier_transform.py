...
from ..field import Field, NumpyField, TensorFlowField
...
class MatrixFourierTransform(FourierTransform):
	...
	def forward(self, field):
		...
		if field.backend == 'numpy':
			...
			res = NumpyField(res, self.output_grid)
	...
	def backward(self, field):
		...
		if field.backend == 'numpy':
			...
			res = NumpyField(res, self.input_grid)
		elif field.backend == 'tensorflow':
			import tensorflow as tf
	...
	def tf_M(self):
		...
		if self._tf_M is None:
			self._tf_M = tf.convert_to_tensor(self.M)
		return self._tf_M
	...
	def tf_M1(self):
		...
		if self._tf_M1 is None:
			self._tf_M1 = tf.convert_to_tensor(self.M1)
		return self._tf_M1
	...
	def tf_M2(self):
		...
		if self._tf_M2 is None:
			self._tf_M2 = tf.convert_to_tensor(self.M2)
		return self._tf_M2
