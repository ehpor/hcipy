...
## Calibrate the DM influence matrix
Influence = shack_hartmann_calibrator(wf, shwfs, shwfse, det, zernike_freeform, 0.1)
...
for t in times:
    ...
    ao_img = wfao.intensity
    
    ## Estimate the shifts from the microlens image
    imshift = shwfse.estimate([wfmla_img]) - lenslet_centers
    
    ## Find the phase of the wavefront by reconstruction
    act_levels = np.dot(Response, imshift)
