...
def make_spider_infinite(p,angle,spider_width):
	...
	def func(grid):
		delta_x = np.max(grid.x)-np.min(grid.x)
		delta_y = np.max(grid.y)-np.min(grid.y)
		spider_length = np.sqrt((delta_x)**2+(delta_y)**2)
...
def obstructed_circular_aperture(pupil_diameter, central_obscuration_ratio,num_spiders =0,spider_width = 0.01):
