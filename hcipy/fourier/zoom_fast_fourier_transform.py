...
class ZoomFastFourierTransform(FourierTransform):
    ...
    def _compute_shifts_and_weights(self, dtype):
        ...
        if complex_dtype != self._current_dtype:
            ...
            w = xp.exp(-1j * self.output_grid.delta * self.input_grid.delta)
            a = xp.exp(1j * self.output_grid.zero * self.input_grid.delta)

            inv_w = xp.exp(1j * self.input_grid.delta * self.output_grid.delta)
            inv_a = xp.exp(-1j * self.input_grid.zero * self.output_grid.delta)
