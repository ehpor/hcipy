...
def test_vector_vortex_coronagraph_polarized():
    ...
    for charge in [2]:
        ...
        assert np.sum(img.intensity) == (-1) * np.sum(img.V)

        # Backward check
        wf_b = prop.backward(img)
        wf_input_b = vortex.backward(wf_b)
        assert np.sum(wf_input_b.intensity) == (-1)**2 * np.sum(wf_input_b.V)
...
def test_scalar_vortex_coronagraph_polarized():
    ...
    for charge in [2]:
        ...
        assert np.sum(img.intensity) == np.sum(img.V)

        wf_b = prop.backward(img)
        wf_input_b = vortex.backward(wf_b)
        assert np.sum(wf_input_b.intensity) == np.sum(wf_input_b.V)
